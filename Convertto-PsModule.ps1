		...
		if (test-GitLocal -ProjectPath $ProjectPath) {
			...
			if (test-GitRemote -ProjectPath $ProjectPath) {
                ...
                $psm1File= $psm1File.Replace("<#{.PSVersionREQ.}#>","$($PSVersionTable.PSVersion.Major)")
                 ...
                 $functionDocumentation | where {$_.Private -eq $false} | foreach { ". `$PSScriptRoot\public\$($_.name ).ps1" | Out-File -Append "$($SourceFile.Directory.FullName)\Module\$($ModuleName)\$($ModuleName).psm1" }
                 $functionDocumentation | where {$_.Private -eq $true} | foreach { ". `$PSScriptRoot\private\$($_.name ).ps1" | Out-File -Append "$($SourceFile.Directory.FullName)\Module\$($ModuleName)\$($ModuleName).psm1" }
                 $functionDocumentation | foreach { "Export-ModuleMember $($_.name )" | Out-File -Append "$($SourceFile.Directory.FullName)\Module\$($ModuleName)\$($ModuleName).psm1" }
                 "}" | Out-File -Append "$($SourceFile.Directory.FullName)\Module\$($ModuleName)\$($ModuleName).psm1" 
                #endregion
				...
				New-ModuleManifest -Path "$($SourceFile.Directory.FullName)\Module\$($ModuleName)\$($ModuleName).psd1" `
                    ...
                    -FunctionsToExport $($functionDocumentation | where {$_.Private -eq $false}).name`
                    -FileList $filelist
